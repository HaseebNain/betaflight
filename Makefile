...
FC_VER_THRUST_MAJOR := $(shell grep " FC_VERSION_THRUST_DEV_LEVEL_MAJOR" src/main/target/$(BASE_TARGET)/version.h | awk '{print $$3}' )
FC_VER_THRUST_MINOR := $(shell grep " FC_VERSION_THRUST_DEV_LEVEL_MINOR" src/main/target/$(BASE_TARGET)/version.h | awk '{print $$3}' )

FC_VER := $(FC_VER_MAJOR).$(FC_VER_MINOR).$(FC_VER_PATCH).$(FC_VER_THRUST_MAJOR).$(FC_VER_THRUST_MINOR)
